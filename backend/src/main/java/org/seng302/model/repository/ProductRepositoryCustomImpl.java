...
public class ProductRepositoryCustomImpl implements ProductRepositoryCustom {
    ...
    /**
     * Search for products for a business by selected fields and barcode
     * @param search list of values to search for
     * @param fields list of fields to search ("id", "name", "manufacturer", "description")
     * @param businessId id of business
     * @param pageable Pageable object
     ...
     */
    @Override
    public Page<Product> findAllProductsByBusinessIdAndIncludedFieldsAndBarcode(List<String> search,
                                                                                List<String> fields,
                                                                                Integer businessId,
                                                                                Pageable pageable,
                                                                                String barcode) {
        ...
        Path<String> businessPath = product.get("businessId");
        Path<String> barcodePath = product.get("barcode");

        // the where clause of the query
        query.where(criteriaBuilder.and(criteriaBuilder.equal(businessPath, businessId),
                                        criteriaBuilder.equal(barcodePath, barcode),
                                        criteriaBuilder.or(predicates.toArray(new Predicate[predicates.size()])))
                   );
